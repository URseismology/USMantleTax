      ...
      function [ output_args ] = write_sac_out(writeDirectory, traces, verbosity)
         ...
         function write_trace_header_to_sac(fid, javaTrace)
            ...
            elseif isstruct(javaTrace)
               ...
               %@OLUGBOJI --- hardwire new fields to see if works
               %outval = single(9);
               %disp('!!!!!!!!!!!!!!!!!!!!!!!!')
               write_sac_field(fid, 'BAZ', javaTrace.BAZ); % Event baz
               write_sac_field(fid, 'GCARC', javaTrace.GCARC) % distance deg
               write_sac_field(fid, 'MAG', javaTrace.MAG)
               write_sac_field(fid, 'EVDP', javaTrace.EVDP)
               write_sac_field(fid, 'EVLA', javaTrace.EVLA)
               write_sac_field(fid, 'EVLO', javaTrace.EVLO) % other details ...
               write_sac_field(fid, 'T0', javaTrace.T0) % other details ...
               write_sac_field(fid, 'T1', javaTrace.T1) % other details ...
               write_sac_field(fid, 'KT1', javaTrace.KT1) % other details ...
               write_sac_field(fid, 'T2', javaTrace.T2) % other details ...
               write_sac_field(fid, 'KT2', javaTrace.KT2) % other details ...
               write_sac_field(fid, 'USER0', javaTrace.USER0) % other details ...
               write_sac_field(fid, 'CMPAZ', javaTrace.CMPAZ) % other details ...
         ...
         function write_trace(writeDirectory, trace)
            ...
            %@OLUGBOJI -- simple file name matched with catalog
            outputFileName = fullfile(writeDirectory, trace.fName);
