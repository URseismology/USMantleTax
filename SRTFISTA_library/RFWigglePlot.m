...
function RFWigglePlot(R, t, rayP, epiDist, tPhase, tWin, scale, singlePlot, pws, timeRedLine, timeBlackDot)
...
if pws
    ...
    [stackR, ~] = f_pws(RR_forStack', 3); 
    %[stackR] = f_pws2(RR_forStack, 3,10,1); %phase and semblance
    stackR = stackR'; % phase weighted stack
else
...
if singlePlot
    ...
    hold on;
     ...
     % Plotting the red dashed line
    if ~isempty(timeRedLine)
        plot([timeRedLine timeRedLine], ylim, 'r--', 'LineWidth', 1.5);
    end

    hold on;

    % Plotting the black dot
    if ~isempty(timeBlackDot)
        plot(timeBlackDot, 0, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'k');
    end
